...
export function WorkoutLog({ goalId, goalName, onBack, api }: { goalId: number; goalName: string; onBack: () => void; api: WorkoutApi }) {
  ...
  const weekMin = weekSessions.reduce((a, s) => a + (s.duration_minutes ?? 0), 0);
        ...
        {weekSessions.length > 0 && (
          <div className="wl-weeksum">
            ...
            <div className="grow">
              ...
              <div className="muted" style={{ fontSize: 12 }}>{weekSessions.length} из 4 тренировок · {weekMin} мин в зале</div>
            </div>
                      ...
                      <div className="list">
                        {wk.items.map((s) => {
                          ...
                          <div key={s.id} className="card wl-daycard" style={{ padding: 14 }} onClick={() => t && setView({ m: "active", t, existing: s })}>
                            ...
                            <div className="muted" style={{ fontSize: 12, marginTop: 2 }}>{s.set_count} подходов{s.duration_minutes ? ` · ${s.duration_minutes} мин` : ""}</div>
...
function ActiveSession({ template, existing, exMap, api, onBack, onDone, onCancel, onOpenExercise }: {
  ...
  useEffect(() => {
    void Promise.all(template.exercises.map((te) => api.lastSets(te.exercise_id))).then((all) => {
      const sl: LiveSets = {}; const sp: Record<number, WSet[]> = {};
      template.exercises.forEach((te, i) => {
        const last = all[i]; sp[te.exercise_id] = last;
        const baseW = te.coach_target_weight ?? (last[0]?.weight ?? 0);
        sl[te.exercise_id] = Array.from({ length: te.target_sets }, (_, k) => ({
          exercise_id: te.exercise_id, set_index: k, weight: baseW, reps: last[k]?.reps ?? last[0]?.reps ?? te.rep_low, done: isDone,
        }));
      });
      setLive(sl); setPrev(sp);
    });
  }, [template, api, isDone]);
  ...
  const complete = async () => {
    ...
    setTimeout(() => onDone({
      ...
      duration_minutes: 78, review_note: review, coach_note: r.coach_note, set_count: allSets.length,
    }), 1500);
  };
  ...
  return (
    <div className="wl">
      ...
      <div className="wl-body" style={{ paddingBottom: 120 }}>
